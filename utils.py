...
def get_optimizer(config, model):
    ...
    # --- 3. 创建参数组 ---
    param_groups = [
        # 基础参数组：使用 config 中设置的 LR
        {'params': base_params, 'lr': config.lr},
        # 路由器参数组：使用更高的 LR
        {'params': router_params, 'lr': config.lr * router_lr_scale}
    ]
...
class myToTensor:
    ...
    def __call__(self, data):
        ori, ref, tar = data
        return torch.tensor(ori).permute(2,0,1), torch.tensor(ref).permute(2,0,1), torch.tensor(tar).permute(2,0,1)
...
class myRandomHorizontalFlip:
    def __init__(self, p=0.5):
        self.p = p
    def __call__(self, data):
        if random.random() < self.p: return TF.hflip(data)
        else: return data
...
class myRandomVerticalFlip:
    def __init__(self, p=0.5):
        self.p = p
    def __call__(self, data):
        if random.random() < self.p: return TF.vflip(data)
        else: return data
...
class myRandomRotation:
    def __init__(self, p=0.5, degree=[0,360]):
        ...
        self.p = p
    def __call__(self, data):
        if random.random() < self.p: return TF.rotate(data,self.angle)
        else: return data
...
class myNormalize:
    def __init__(self, data_name, train=True):
        if data_name == 'isic18':
            ...
            if train:
                ...
                self.mean = 149.8485
                self.std = 35.3346
        elif data_name == 'BraTs2020_t1_t2_tiny':
            if train:
                self.mean = 0.1614
                self.std = 0.2456
            else:
    ...
    def __call__(self, data):
        ori, ref, tar = data
        ori_normalized = (ori-self.mean)/self.std
        ori_normalized = ((ori_normalized - np.min(ori_normalized)) 
                            / (np.max(ori_normalized)-np.min(ori_normalized))) * 255.
        ref_normalized = (ref-self.mean)/self.std
        ref_normalized = ((ref_normalized - np.min(ref_normalized)) 
                            / (np.max(ref_normalized)-np.min(ref_normalized))) * 255.
        tar_normalized = (tar-self.mean)/self.std
        tar_normalized = ((tar_normalized - np.min(tar_normalized)) 
                            / (np.max(tar_normalized)-np.min(tar_normalized))) * 255.
        return ori_normalized, ref_normalized, tar_normalized
...
def denormalize(img):
    # 适用于 [-1, 1] 范围的图像
    img = ((img + 1.0) / 2.0).clamp(0.0, 1.0) * 255.0
    return img
...
# 计算 PSNR 和 SSIM 以进行评估
def compute_psnr_ssim(ori, sr_output):
    psnr_value = calc_psnr(ori, sr_output)
    ssim_value = calc_ssim(ori, sr_output)
