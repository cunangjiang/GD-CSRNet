...
class setting_config:
    ...
    model_config = {
        ...
        'load_ckpt_path': 'pre_trained_weights/vssmtiny_dp01_ckpt_epoch_292.pth',#vmamba_small_e238_ema.pth',
    }

    datasets = 'BraTs2020_t1_t2' 
    if datasets == 'BraTs2020_t1_t2':
        data_path = 'datasets/BraTs2020_t1_t2/'
    elif datasets == 'IXI_t1_t2':
        data_path = 'datasets/IXI_t1_t2/'
    elif datasets == 'fastmri':
        data_path = 'datasets/fastmri/'
    else:
    ...
    # gradient loss 权重
    # 建议先从 0.05 开始试，再看结果调到 0.02 / 0.1
    gradient_loss_weight = 0.02

    # --- MoE 辅助损失的权重 ---
    # (用于惩罚 STSS2D 和 ARFU_MoE 内部的专家负载不均衡)
    moe_aux_loss_alpha = 1e-5 
    
    # 是否使用临床退化增强
    use_clinical_degradation = None
    ...
    work_dir = 'results/' + network + '_' + datasets + '_' + 'x' + str(upscale) + '/'
    ...
    if opt == 'Adadelta':
        ...
        lr = 0.0001*2 # default: 1e-4 – learning rate
        betas = (0.9, 0.999) # default: (0.9, 0.999) – coefficients used for computing running averages of gradient and its square
        eps = 1e-8 # default: 1e-8 – term added to the denominator to improve numerical stability
